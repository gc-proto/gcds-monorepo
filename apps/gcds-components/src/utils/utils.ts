...
export const closestElement = (selector: string, el: HTMLElement | null): HTMLElement | null => {
  if (el) {
    const rootNode = el.getRootNode() as ShadowRoot | Document;
    return (
      el.closest(selector) ||
      (rootNode && 'host' in rootNode ? closestElement(selector, rootNode.host as HTMLElement) : null)
    );
  }
...
export const elementGroupCheck = (name: string): boolean => {
  let hasCheck = false;
...
export function handleValidationResult(
  element: HTMLElement & { errorMessage?: string },
  ...
  if (!validationResult.valid) {
    element.errorMessage = validationResult.reason[langKey];
    errorEv.emit({
      message: `${label} - ${validationResult.reason[langKey]}`,
    });
 ...
 */
export function formatHTMLErrorMessage(error: string, lang: string, el: any): string {
  const langKey = lang as 'en' | 'fr';
  switch (error) {
    case 'valueMissing':
      return I18N[langKey][error];
    case 'typeMismatch':
      if (el.type === 'url' || el.type === 'email') {
        return I18N[langKey][error][el.type as 'url' | 'email'];
      } else {
        return String(I18N[langKey][error]);
      }
    case 'tooLong':
      return I18N[langKey][error]
        .replace('{max}', String(el.maxlength || el.characterCount))
        .replace('{current}', String(el.value.length));
    case 'tooShort':
      return I18N[langKey][error]
        .replace('{min}', String(el.minlength))
        .replace('{current}', String(el.value.length));
    case 'rangeUnderflow':
      return I18N[langKey][error].replace('{min}', String(el.min));
    case 'rangeOverflow':
      return I18N[langKey][error].replace('{max}', String(el.max));
    case 'stepMismatch':
      const lower = Math.floor(Number(el.value) / Number(el.step)) * Number(el.step);
      const upper = lower + Number(el.step);
      return I18N[langKey][error]
        .replace('{lower}', String(lower))
        .replace('{upper}', String(upper));
    case 'badInput':
